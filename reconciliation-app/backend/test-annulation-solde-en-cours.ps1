# Script de test pour vérifier que les opérations annulées utilisent le solde en cours

$baseUrl = "http://localhost:8080/api"

Write-Host "=== Test du solde en cours pour les opérations annulées ===" -ForegroundColor Green

# 1. Vérifier l'état actuel
Write-Host "`n1. État actuel du compte:" -ForegroundColor Yellow
...
try {
    ...
    if ($compte) {
        Write-Host "Compte: $($compte.numeroCompte)" -ForegroundColor Cyan
        Write-Host "Solde en cours: $($compte.solde) FCFA" -ForegroundColor Green
        ...
        $operationTest = @{
            ...
            montant = 10000
            banque = "TEST_BANK"
            nomBordereau = "TEST_SOLDE_EN_COURS_$(Get-Date -Format 'yyyyMMddHHmmss')"
        ...
        if ($cancelResponse -eq $true) {
            ...
            Write-Host "`n4. Vérification de l'opération annulée:" -ForegroundColor Yellow
            ...
            # Vérifier que le soldeAvant correspond au solde en cours
            Write-Host "`nVérification du solde en cours:" -ForegroundColor Cyan
            Write-Host "  - Solde avant (opération): $($opAnnulee.soldeAvant) FCFA" -ForegroundColor White
            Write-Host "  - Solde en cours (compte): $($compteAfterCreate.solde) FCFA" -ForegroundColor White
            
            if ($opAnnulee.soldeAvant -eq $compteAfterCreate.solde) {
                Write-Host "  ✅ Le solde avant correspond au solde en cours" -ForegroundColor Green
            } else {
                Write-Host "  ❌ Le solde avant ne correspond pas au solde en cours" -ForegroundColor Red
                Write-Host "  - Différence: $($opAnnulee.soldeAvant - $compteAfterCreate.solde) FCFA" -ForegroundColor Red
            }
            ...
            Write-Host "`n5. Vérification du solde du compte après annulation:" -ForegroundColor Yellow
            ...
            Write-Host "`n6. Vérification des opérations d'annulation:" -ForegroundColor Yellow
