# Script de test pour vérifier que les opérations annulées ont leur soldeApres correctement calculé

$baseUrl = "http://localhost:8080/api"

Write-Host "=== Test du soldeApres pour les opérations annulées ===" -ForegroundColor Green
...
try {
    ...
    if ($compte) {
        ...
        $operationTest = @{
            ...
            montant = 25000
            banque = "TEST_BANK"
            nomBordereau = "TEST_SOLDE_APRES_$(Get-Date -Format 'yyyyMMddHHmmss')"
        ...
        if ($cancelResponse -eq $true) {
            ...
            $impactAttendu = $opAnnulee.soldeAvant + $opAnnulee.montant  # Pour total_cashin, l'impact est -montant
            $impactReel = $opAnnulee.soldeApres - $opAnnulee.soldeAvant
            
            Write-Host "`nVérification du soldeApres:" -ForegroundColor Cyan
            Write-Host "  - Solde avant: $($opAnnulee.soldeAvant) FCFA" -ForegroundColor White
            Write-Host "  - Solde après: $($opAnnulee.soldeApres) FCFA" -ForegroundColor White
