...
async function updateModelWithUniqueSteps(modelId, updatedModel) {
  try {
...
function createNewStepsForModel(model) {
  ...
  // Vérifier les actions déjà présentes
  const existingActions = model.processingSteps.map(step => step.action);
  
  // Créer les nouvelles étapes seulement pour les actions manquantes
  newActions.forEach((action) => {
    // Vérifier si l'action existe déjà
    if (existingActions.includes(action.name)) {
      console.log(`⚠️ Action ${action.name} déjà présente dans le modèle ${model.name}`);
      return;
    }
    ...
    if (fields.length > 0) {
      newSteps.push({
        stepId: `step_${action.name}_${uuidv4()}`, // Utiliser UUID pour garantir l'unicité
...
async function addActionsToAllModels() {
  ...
  try {
    ...
    for (const model of models) {
      ...
      // Créer les nouvelles étapes pour ce modèle (avec vérification des doublons)
      ...
      const result = await updateModelWithUniqueSteps(model.id, updatedModel);
      if (result) {
        successCount++;
...
async function testSimpleUpdate() {
  try {
    ...
    if (testModel) {
      ...
      // Vérifier si l'action existe déjà
      const hasAction = testModel.processingSteps.some(step => step.action === 'normalizeHeaders');
      if (hasAction) {
        console.log('⚠️ Action normalizeHeaders déjà présente, test ignoré');
        return;
      }
      
      // Ajouter une seule nouvelle étape pour tester
      const newStep = {
        stepId: `step_test_${uuidv4()}`, // Utiliser UUID
        name: 'TEST_ACTION',
        type: 'format',
      ...
      const result = await updateModelWithUniqueSteps(testModel.id, updatedModel);
      if (result) {
