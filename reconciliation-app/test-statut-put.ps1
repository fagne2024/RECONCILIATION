# Test de l'endpoint PUT pour la mise à jour du statut
Write-Host "=== Test de mise à jour du statut avec PUT ===" -ForegroundColor Green
...
# Test 3: Test de mise à jour du statut avec PUT
Write-Host "`n2. Test de mise à jour du statut avec PUT..." -ForegroundColor Yellow

$requestBody = @{
    statut = "TRAITE"
} | ConvertTo-Json -Depth 1

Write-Host "📤 Envoi de la requête PUT..." -ForegroundColor Cyan
...
try {
    ...
    $result = Invoke-RestMethod -Uri "$baseUrl/api/ecart-solde/$ecartId/statut" -Method PUT -Body $requestBody -Headers $headers
